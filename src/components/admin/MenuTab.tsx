                ...
                <div className="mb-3">
                  {item.discount_id && (
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm text-slate-500 line-through">{formatRupiah(item.base_price || item.price)}</span>
                      <span className="px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full">Diskon</span>
                    </div>
                  )}
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Round to nearest 1000 if not already multiple of 1000
                    const roundedValue = Math.round(value / 1000) * 1000;
                    setOptionItemFormData(prev => ({ ...prev, additional_price: Math.max(0, roundedValue) }));
                  }}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  placeholder="0"
                />
                <p className="text-xs text-slate-500 mt-1">Biaya tambahan dalam kelipatan Rp 1.000 (otomatis dibulatkan)</p>
              </div>
